...
export default function TrimestralPrintTable({ trimester, rows, validateUnitsPerSemester }) {
	const { totalUnits } = computeTotals(rows);
	const validation = validateUnitsPerSemester(rows);
	return (
		<div className="overflow-x-auto print-trimester" style={{marginBottom: '16px'}}>
			<table className="w-full text-xs border" style={{width: '100%', borderCollapse: 'collapse', color: '#000'}}>
				<thead>
					<tr className="text-gray-900" style={{background: '#fff', color: '#000'}}>
						<th className="p-0.5 text-center text-xs font-bold" colSpan={8} style={{padding: '6px', border: '1px solid #111', verticalAlign: 'top', color: '#000'}}>{trimester}</th>
					</tr>
